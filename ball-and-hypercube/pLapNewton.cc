 ...
 *
 * Finite element solver for 2D and 3D p-Laplace problems with either
...
// ******************************************************************************************//
  ...
  double Solution<dim>::value (const Point<dim> &pto,const unsigned int) const
  {
      ...
      else if (domain_id == 0 && gamma_1 == 0 && gamma_2 == 0 && rhs ==1) {
          double r=sqrt(pto.square());
          return 1-r;
      }
      
      else if (gamma_1 == 0 && gamma_2 == 1 && rhs ==0){
          return (std::abs(pto(0)) -  std::abs(pto(1)));
      }
      
      else if (gamma_1 == 0 && gamma_2 == 2 && rhs ==0){
  ...
  Tensor<1,dim> Solution<dim>::gradient (const Point<dim> &pto,const unsigned int) const
  {
      if (gamma_1 == 1 && gamma_2 == 3 && rhs ==1){
          ...
          return -1.0*pto/r;
           ...
           Tensor<1,dim> grd;
           grd[0] = pto(0) * std::pow(std::abs(pto(0)), -1.0);
           grd[1] = -pto(1) * std::pow(std::abs(pto(1)), -1.0);
          return grd;
      }
    ...
    if (gamma_1 == 1)
{
  ...
  auto is_origin = [&](const Point<dim> &p) {
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    for (const auto v : GeometryInfo<dim>::vertex_indices())
      if (is_origin(cell->vertex(v)))
      {
        const unsigned int dof_index = cell->vertex_dof_index(v, 0);
        hanging_node_constraints.add_line(dof_index);
      }
}
    ...
    if (domain_id==0)
      {
        ...
        // For the unit ball around the origin
        GridGenerator::hyper_ball(triangulation);
        static const SphericalManifold<dim,spacedim> boundary;
  ...
  try
    {
      ...
      param.read_parameters("PARAMS.prm");
        prm.enter_subsection ("Mesh & Refinement Parameters");
        const int dim_id=prm.get_integer("Dimension");
        prm.leave_subsection ();
        if (dim_id == 2)
        {
