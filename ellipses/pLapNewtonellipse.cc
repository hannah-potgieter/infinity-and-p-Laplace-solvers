 ...
 *
 * Finite element solver for 2D p-Laplace problems on 2D ellipses with either
 * homogeneous Dirichlet boundary conditions or mixed Dirichlet/Neumann
 * conditions. For selected choices of right-hand side and boundary data,
 * closed-form reference solutions are available in the limiting case
 * p -> infinity and are used for verification.
 *
...
// Constructor
  ...
  void ParameterReader::declare_parameters()
  {
    ...
    prm.enter_subsection ("Mesh & Refinement Parameters");
    {
     ...
     prm.declare_entry("Code for the natural boundary", "0",Patterns::Integer(0,2),
...
// ******************************************************************************************//
  ...
  {
  public:
    Solution (const unsigned int &gamma1_value, const unsigned int &gamma2_value, const unsigned int &rhs_value, const unsigned int &domain) : Function<dim>(),
      gamma_1(gamma1_value), gamma_2(gamma2_value), rhs(rhs_value), domain_id(domain){}
    ...
    private:
      const unsigned int gamma_1;
      const unsigned int gamma_2;
  ...
  {
      if (gamma_1 == 1 && gamma_2 == 2 && rhs ==1){
          double r=sqrt(pto.square());
          return r;
      }
      
     
      else if (gamma_1 == 0 && gamma_2 == 1 && rhs ==0){
...
//                              Boundary condition
  ...
  {
  public:
    BoundaryValues (const unsigned int &gamma2_value) : Function<dim>(), gamma_2(gamma2_value){}
...
// fill-in entry for the solution
  ...
  void pPoisson<spacedim>::process_solution (const unsigned int it)
  {
    Vector<float> difference_per_cell (triangulation.n_active_cells());
      ...
      const int gamma_1=prm.get_integer("Code for the feature boundary");
    ...
    VectorTools::integrate_difference (mapping, dof_handler,present_solution,
                                       Solution<spacedim>(gamma_1, gamma_2, rhs, domain_id),difference_per_cell,QGauss<dim>(3),VectorTools::L2_norm);
    L2_error = difference_per_cell.l2_norm();

    // compute H1 seminorm error (save to difference_per_cell)
    VectorTools::integrate_difference (mapping, dof_handler,present_solution,Solution<spacedim>(gamma_1, gamma_2, rhs, domain_id),
...
// unchanged from step-15
  ...
  void pPoisson<spacedim>::setup_system (const bool initial_step)
  {
	...
	double min_r2 = 0.0;
	types::global_dof_index closest_dof = numbers::invalid_dof_index;

	for (const auto &cell : dof_handler.active_cell_iterators())
	  for (const auto v : GeometryInfo<dim>::vertex_indices())
	  {
	    const Point<dim> p = cell->vertex(v);
	    const double r2 = p.square(); // ||p||^2

	    const auto dof_index = cell->vertex_dof_index(v, /*component=*/0);

	    if (!have_candidate || r2 < min_r2)
	    {
	      have_candidate = true;
	      min_r2 = r2;
	...
	AssertThrow(have_candidate && closest_dof != numbers::invalid_dof_index,
		    ExcMessage("No vertex DoF found to constrain."));

	hanging_node_constraints.add_line(closest_dof);
	}
...
// Dump in vtu format for visualization
  ...
  void pPoisson<spacedim>::output_results (unsigned int counter) const
  {
    ...
    // compute L2 error (save to difference_per_cell)
    VectorTools::integrate_difference (mapping, dof_handler,present_solution,
                                       Solution<spacedim>(gamma_1, gamma_2, rhs, domain_id),difference_per_cell,QGauss<dim>(3),VectorTools::Linfty_norm);
    ...
    std::map<types::global_dof_index, double> boundary_values;
      ...
      if (gamma_2 != 2)
      {
          VectorTools::interpolate_boundary_values (dof_handler,
                                                 0,
                                                 BoundaryValues<spacedim>(gamma_2),
                                                 boundary_values);
      }
  ...
  try
    {
      ...
      param.read_parameters("PARAMSellipse.prm");
            std::cout << "Instantiating pPoisson<2>" << std::endl;
            pPoisson<2> pPoissonProblem(prm);
            pPoissonProblem.run ();
