 ...
 * with zero Dirichlet boundary conditions on a torus (R = 2, r = 1) in 3D.
 * As p approaches infinity, the solution converges toward the distance-to-
 * boundary function, which is used for comparison.
 *
...
// ******************************************************************************************//
  ...
  double Solution<dim>::value (const Point<dim> &pto,const unsigned int) const
  {
  	...
  	double rho = std::sqrt(pto[0]*pto[0]+pto[2]*pto[2]);
  	
  	
          return r-std::sqrt((rho-R)*(rho-R)+pto[1]*pto[1]);
  ...
  Tensor<1,dim> Solution<dim>::gradient (const Point<dim> &pto,const unsigned int) const
  {
  	...
  	double rho = std::sqrt(pto[0]*pto[0]+pto[2]*pto[2]);
          ...
          Tensor<1,dim> grd;
          grd[0] = (rho-R)/rho*pto[0];
          grd[1] = pto[1];
          grd[2] = (rho-R)/rho*pto[2];
          
         return -(1.0 / std::sqrt((rho-R)*(rho-R)+pto[1]*pto[1])) * grd;
...
// uses quadratic interpolation
  ...
  void pPoisson<spacedim>::init_mesh ()
  {
    ...
    prm.leave_subsection ();
        ...
        // For torus with R = 2, r = 1
        Triangulation<spacedim-1, spacedim> bdry;
        GridGenerator::torus(triangulation, 2.0, 1.0);
         ...
         GridGenerator::extract_boundary_mesh(triangulation,
          ...
          bdry.set_all_manifold_ids(0);
          bdry.set_manifold(0, TorusManifold<spacedim-1>(2.0, 1.0));
  ...
  try
    {
      ...
      param.read_parameters("PARAMStorus.prm");
        prm.enter_subsection ("Mesh & Refinement Parameters");
        prm.leave_subsection ();
