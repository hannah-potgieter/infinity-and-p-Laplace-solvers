"""
Plot figures from saved .npz data for Section 6.2 (Infinity Laplacian).

Reads from:  outputs/expr_6_2/{example}/{run_tag}/npy/
Saves to:    outputs/images/

Usage:
    python plot/experiment_6_2_plot.py --example arctan
    python plot/experiment_6_2_plot.py --example all
    python plot/experiment_6_2_plot.py --example absolute --run-tag normal
    python plot/experiment_6_2_plot.py --example all --base-dir outputs/expr_6_2
"""
...
def plot_training_history(history_path, out_path):
    ...
    ax.semilogy(data['bc_loss'], label=r'$\mathrm{MSE}_{\partial\Omega}$', color='blue', alpha=0.8)
    ax.semilogy(data['pde_loss'], label=r'$\mathrm{MSE}_{\mathrm{PINNs}}$', color='orange', alpha=0.8)
    ax.semilogy(data['test_loss'], label=r'$\mathrm{MSE}_{\infty}$', color='green', linewidth=1.5)

    val_loss = data.get('val_loss', None)
    if val_loss is not None and not np.all(np.isnan(val_loss)):
        ax.semilogy(
            val_loss,
            label=r'$\mathrm{MSE}_{\partial\Omega}^{\mathrm{val}}$',
            color='deeppink',
            linewidth=1.5,
        )
        best_epoch = int(np.nanargmin(val_loss))
        ax.axvline(x=best_epoch, color='deeppink', linestyle='--', alpha=0.5,
                   label=rf'$\mathrm{{Selected\ epoch\ ({best_epoch + 1})}}$')

    if 'interface_loss' in data:
        intf = data['interface_loss']
        if np.any(intf > 0):
            ax.semilogy(intf, label=r'$\mathrm{MSE}_{\mathrm{interface}}$', color='cyan', alpha=0.8)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Plot figures from saved .npz data for Section 6.2',
    ...
    parser.add_argument('--base-dir', type=str, default='outputs/expr_6_2',
                        help='Base directory for Section 6.2 outputs')
